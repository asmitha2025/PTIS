...
const STATIC_PREFIX = location.pathname.includes("/frontend/") ? "../" : "./";

function staticAsset(path) {
  return STATIC_PREFIX + path.replace(/^\/+/, "");
}
...
async function loadAllEvidence() {
  const [scenario, suite, batch, extremeBatch, official, cctv, field, remoteAggregate, routeGeometry] = await Promise.all([
    fetchJson("/api/evidence/latest", staticAsset("evidence/latest_run.json"), fallbackEvidence),
    fetchJson("/api/evidence/suite", staticAsset("evidence/suite_report.json"), fallbackSuite),
    fetchJson("/api/evidence/batch", staticAsset("evidence/batch_report.json"), fallbackBatch),
    fetchJson("/api/evidence/extreme-batch", staticAsset("evidence/extreme_batch_report.json"), fallbackExtremeBatch),
    fetchJson("/api/official-reference", staticAsset("data/official_reference_bengaluru_cmp_2020.json"), fallbackOfficial),
    fetchJson("/api/evidence/cctv", staticAsset("evidence/cctv_bmd45_report.json"), fallbackCctv),
    fetchJson("/api/evidence/field-replay", staticAsset("evidence/field_replay_report.json"), fallbackField),
    fetchJson("/api/evidence/remote-aggregate", staticAsset("evidence/remote_aggregate_replay_report.json"), fallbackRemoteAggregate),
    fetchJson("/api/route-geometry", staticAsset("data/orr_silk_board_whitefield_route_osrm.geojson"), fallbackRouteGeometry)
  ]);
...
function renderCctv(cctv) {
  ...
  const canShowLocalFrames = location.pathname.includes("/frontend/");
  document.getElementById("cctv-strip").innerHTML = (cctv.sample_images || []).slice(0, 3).map(item => {
    const src = encodeURI(staticAsset(`Real data/BMD-45-Val/${item.file_name}`));
    const frame = canShowLocalFrames
      ? `<img src="${escapeHtml(src)}" alt="BMD-45 validation frame ${escapeHtml(item.file_name)}" />`
      : `<div class="cctv-frame-placeholder"><span>BMD-45</span><strong>${integer(item.coco_annotation_count)}</strong></div>`;
    return `
    ...
    `;
  }).join("") || `<figure class="cctv-frame"><div class="cctv-frame-placeholder"><span>BMD-45</span><strong>--</strong></div><figcaption><span>waiting</span><strong>--</strong></figcaption></figure>`;
}
